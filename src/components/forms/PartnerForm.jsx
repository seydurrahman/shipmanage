...
export default function PartnerForm() {
  ...
  const getShipCost = (shipId) =>
    ships.find((s) => s.id === parseInt(shipId))?.purchase_cost || 0;
  ...
  ========================= */
  useEffect(() => {
    if (!formData.ship || !formData.share_amount) {
      setFormData((prev) => ({ ...prev, share_percentage: "" }));
      return;
    }

    const purchaseCost = parseFloat(getShipCost(formData.ship));
    const shareAmount = parseFloat(formData.share_amount);

    if (purchaseCost > 0 && shareAmount >= 0) {
      const percent = ((shareAmount / purchaseCost) * 100).toFixed(2);
      setFormData((prev) => ({
        ...prev,
        share_percentage: percent,
      }));
    }
  }, [formData.ship, formData.share_amount]);
  ...
  const submit = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        share_amount: parseFloat(formData.share_amount),
                ...
                <table className="w-full text-white min-w-[800px]">
                  <thead className="bg-white/20">
                    <tr>
                      ...
                      <th className="p-2 text-left">Share Amount</th>
                  ...
                  <tbody>
                    {partners.map((p) => (
                      <tr key={p.id} className="hover:bg-white/10">
                        ...
                        <td className="p-2">
                          {p.share_amount
                            ? `Tk ${Number(p.share_amount).toFixed(2)}`
                            : "—"}
                        </td>
                        <td className="p-2">
