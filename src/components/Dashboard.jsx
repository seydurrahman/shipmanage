...
const BarChartComponent = React.memo(
  ({ ships, dailyIncomes, selectedDate }) => {
    ...
    const chartData = ships.map((ship) => {
      const values = displayedDays.map((day) => {
        ...
        return incomes.reduce(
          (s, x) => s + parseFloat(x.actual_amount || 0),
          0
        );
      });
    ...
    // Center the view around the selected date showing 3 days before and 3 days after
    React.useEffect(() => {
      const dayStr = format(selectedDate, "yyyy-MM-dd");
      let idx = daysInMonth.findIndex(
        (d) => format(d, "yyyy-MM-dd") === dayStr
      );

      // If selected date is not in the month's days (edge cases), pick the nearest
      if (idx === -1) {
        const selectedMs = new Date(dayStr).getTime();
        idx = daysInMonth.reduce((bestIdx, d, i) => {
          const diff = Math.abs(new Date(d).getTime() - selectedMs);
          const bestDiff =
            bestIdx === -1
              ? Infinity
              : Math.abs(new Date(daysInMonth[bestIdx]).getTime() - selectedMs);
          return diff < bestDiff ? i : bestIdx;
        }, -1);
        if (idx === -1) idx = 0;
      }

      const desiredStart = Math.max(0, idx - Math.floor(daysPerView / 2));
      const maxStart = Math.max(0, daysInMonth.length - daysPerView);
      setScrollPosition(Math.min(desiredStart, maxStart));
    }, [selectedDate, daysInMonth.length]);
...
const Dashboard = () => {
  ...
  const calculateProfit = async (date) => {
    try {
      setLoading(true);
      await api.post("profits/calculate_profit/", {
        month: format(date, "yyyy-MM"),
      });

      await fetchProfits(); // refresh list
      alert("Monthly profit calculated successfully!");
    } catch (err) {
  ...
  const todayTotal = dailyIncomes
    ...
    .reduce((s, x) => s + parseFloat(x.actual_amount || 0), 0);
          ...
          <div className="flex justify-between mb-4">
            ...
            <button
              ...
              className={`px-4 py-2 rounded text-sm md:text-base ${
                loading ? "bg-gray-400 text-gray-200" : "bg-blue-600 text-white"
              }`}
            >
                  ...
                  profitList.map((p) => (
                    <tr key={p.id} className="border-t">
                      ...
                      <td className="px-3 py-2 text-right">
                        Tk {parseFloat(p.total_income || 0).toFixed(2)}
                      </td>
                      <td className="px-3 py-2 text-right">
                        Tk {parseFloat(p.total_expenses || 0).toFixed(2)}
                      </td>
                      <td className="px-3 py-2 text-right">
                        <span
                          className={
                        ...
                        >
                          Tk {parseFloat(p.net_profit || 0).toFixed(2)}
                        </span>
                      </td>
