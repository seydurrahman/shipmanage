...
const PartnerReport = () => {
  ...
  const calculateShipIncome = (shipId) => {
    ...
    // Calculate total income (use actual_amount when available)
  ...
  ships.forEach((ship) => {
    ...
    shipPartners.forEach((p) => {
      // Use Share Amount provided in PartnerForm (partner.share_amount)
      const shareAmount = parseFloat(p.share_amount || 0);

      // Payable amount = partner's share percentage of net income
      const payable = (income / purchaseCost) * parseFloat(p.share_amount || 0);
          ...
          {slips.map((slip, index) => (
            ...
            >
              <div>
                ...
                <p>
                  <strong>Share Amount:</strong> Tk{" "}
                  {slip.shareAmount.toFixed(2)}
                </p>

                <p>
                  <strong>Income:</strong> Tk {slip.income.toFixed(2)}
                </p>
                <p>
